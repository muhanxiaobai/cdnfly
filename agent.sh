...
if [[ $MASTER_VER == "" ]]; then
    ...
    agent_ver=`(curl -s -m 5 "http://auth.muhanyun.cn/master/upgrades?version_num=$version_num" || curl -s -m 5 "https://auth.muhanyun.cn/master/upgrades?version_num=$version_num") | grep -Po '"agent_ver":"\d+"' | grep -Po "\d+" || true`
    if [[ "$agent_ver" == "" ]]; then
        echo "无法获取agent版本"
        exit 1
    fi
...
download "https://cdnfly.muhanyun.cn/cdnflydadao/raw/main/agent/$tar_gz_name" "https://cdnfly.muhanyun.cn/cdnflydadao/raw/main/agent/$tar_gz_name" "$tar_gz_name"
